...
const SESService = (config) => {
    ...
    const createMimeMessage = (data, defaultSender, toAddresses, ccAddresses, attachments) => {
        return new Promise((resolve, reject) => {
            ...
            const mailOptions = {
                ...
                subject: mapSubject(data.Subject, toAddresses, ccAddresses),
                html: data.Body,
    ...
    _sesService.send = ({ data, defaultSender = config.DefaultSender, topicArn = config.ArnPath }) => {
        return new Promise((res, rej) => {
            ...
            const mappedToAddresses = mapOutboundAddress(toAddresses);
            const mappedCcAddresses = mapOutboundAddress(ccAddresses);
            ...
            if (data.Attachments && data.Attachments.length > 0) {
                ...
                createMimeMessage(
                    ...
                    const params = {
                        ...
                        Destinations: [...mappedToAddresses, ...(mappedCcAddresses || [])]
                    };
                    ...
                    rej(new Error(`Failed to create MIME message: ${error.message}`));
                });
            } else {
                let params = {
                    Destination: {
                        BccAddresses: mappedToAddresses,
                        CcAddresses: mappedCcAddresses,
                    },
                    Message: {
                        Body: {
                            Html: {
